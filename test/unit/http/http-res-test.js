...
test('Architect v5 + Functions', (t) => {
  ...
  run(responses.arc5.cacheControl, (err, res) => {
    ...
    if (responses.arc5.cacheControl.headers['cache-control'] && !res.headers['cache-control']) t.pass(`Headers normalized and de-duped: ${str(res.headers)}`)
    t.equal(res.statusCode, 200, 'Responded with 200')
  })
...
test('Architect v4 + Functions statically-bound content type responses', (t) => {
  ...
  let run = (response, data, contentType) => {
    ...
    handler(request, {}, (err, res) => {
      ...
      if (res.headers['Content-Type'].includes('json')) t.equal(str(data), res.body, match('res.body', res.body))
      else t.equal(str(data), str(res.body), match('res.body', res.body))
