...
let bodyParser = require('../../../http/helpers/body-parser')
let interpolate = require('../../../http/helpers/params')
...
test('Architect v6: get /', (t) => {
  ...
  let handler = http((req, res) => {
    ...
    if (unNulled(request.body, req.body)) t.pass(match('req.body', req.body))
    ...
    if (request.httpMethod === req.method) t.equal(req.httpMethod, req.method, match('req.method/httpMethod', req.method))
    // Compare mutation of nulls into objects, and
    // also compare interpolation to nicer, backwards compat req params
    if (unNulled(request.pathParameters, req.params)) t.equal(req.pathParameters, req.params, match('req.params/pathParameters', req.params))
    if (unNulled(request.queryStringParameters, req.query)) t.equal(req.queryStringParameters, req.query, match('req.query/queryStringParameters', req.query))
...
test('Architect v6: get /?whats=up', (t) => {
  ...
  let handler = http((req, res) => {
    ...
    if (request.httpMethod === req.method) t.equal(req.httpMethod, req.method, match('req.method/httpMethod', req.method))
    if (unNulled(request.pathParameters, req.params)) t.equal(req.pathParameters, req.params, match('req.params/pathParameters', req.params))
    if (request.queryStringParameters === req.query) t.equal(req.queryStringParameters, req.query, match('req.query/queryStringParameters', req.query))
    t.ok(req.session, 'req.session is present')
    res(basicResponse)
  })
  handler(request, {}, end)
...
test('Architect v6: get /nature/hiking', (t) => {
  ...
  let handler = http((req, res) => {
    ...
    t.equal(str(request.headers), str(req.headers), match('req.headers', req.headers))
    if (request.httpMethod === req.method) t.equal(req.httpMethod, req.method, match('req.method/httpMethod', req.method))
    if (str(request.pathParameters) === str(req.pathParameters)) t.equal(str(req.params), str(req.pathParameters), match('req.params/pathParameters', req.params))
    if (unNulled(request.queryStringParameters, req.query)) t.equal(req.queryStringParameters, req.query, match('req.query/queryStringParameters', req.query))
    t.ok(req.session, 'req.session is present')
    res(basicResponse)
  })
...
test('Architect v5: get /', (t) => {
  ...
  let handler = http((req, res) => {
    ...
    if (str(request.query) === str(req.query)) t.equal(str(req.queryStringParameters), str(req.query), match('req.query/queryStringParameters', req.query))
    t.ok(req.session, 'req.session is present')
    res(basicResponse)
  })
  handler(request, {}, end)
